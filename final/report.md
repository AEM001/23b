...
为了更精确地进行海域区域划分和测线规划，我们首先使用`scipy.interpolate.griddata`函数将水深数据进行网格化处理，并在此基础上量化了海底地形的关键特征，即**坡度**和**坡向**。这些地形特征是后续K-Means聚类和递归分割算法的核心输入。

在代码实现中，我们首先将原始的散点水深数据插值成均匀的格网化数据 $z(x,y)$。随后，我们利用`numpy`库的`gradient`函数计算水深在X和Y方向上的空间偏导数（梯度）：

$$\frac{\partial z}{\partial x} = \frac{z(x+\Delta x, y) - z(x-\Delta x, y)}{2\Delta x}$$

$$\frac{\partial z}{\partial y} = \frac{z(x, y+\Delta y) - z(x, y-\Delta y)}{2\Delta y}$$

基于这些梯度信息，我们计算了两个重要的地形参数：

*   **坡度 ($S$)**：反映海底地势的陡峭程度，数值越大表示地形越崎岖。
    $$S = \sqrt{(\frac{\partial z}{\partial x})^2+(\frac{\partial z}{\partial y})^2}$$

*   **坡向 ($\phi$)**：指示海底表面最陡峭的倾斜方向，通常以角度表示，能够揭示地形的朝向。
    $$\phi = \arctan2(-\frac{\partial z}{\partial y},-\frac{\partial z}{\partial x})$$
    其中 $\arctan2$ 函数能够正确处理不同象限的角度，确保坡向计算的准确性。

这些量化的地形特征在区域划分的两步法中发挥了关键作用：
*   在**K-Means聚类分析**中，坡度和坡向与坐标信息一同作为聚类的特征，帮助算法识别出地形相似的区域块。
*   在**递归分割算法**中，尤其依赖于**坡向**的方差来指导区域的进一步细分。通过设定"坡向方差阈值(aspect_variance_threshold)"，当某个区域内的坡向变化较大时（即方差超过阈值），该区域将被继续分割，直至满足条件或达到最大递归深度，从而实现地形敏感的、无重叠的矩形划分。

### 2.2 混合式智能区域划分

考虑到单一划分方法的局限性，采用"可视化引导+算法优化"的两阶段划分策略：

**第一阶段：K-means聚类探索**

构建四维特征向量进行聚类分析：
$$\mathbf{F}_i = [x_i, y_i, S_i, \phi_i]^T$$
其中 $(x_i, y_i)$ 为空间坐标，$S_i$ 和 $\phi_i$ 分别为对应点的坡度和坡向。对特征向量进行标准化处理（使用 `StandardScaler`）后，采用K-means算法（通过 `sklearn.cluster.KMeans` 实现）进行聚类：
$$\min \sum_{i=1}^{k} \sum_{\mathbf{x} \in C_i} ||\mathbf{x} - \boldsymbol{\mu}_i||^2$$
此阶段主要用于识别地形的内在结构模式，虽然产生的聚类边界存在重叠，但为后续精确划分提供了重要参考。

**第二阶段：递归分割优化**

基于坡向方差最小化原则，采用递归二分法（通过自定义的 `recursive_split` 函数实现）获得无重叠的矩形区域。

该函数通过递归过程对海域进行划分。对于任一给定区域，其核心逻辑如下：

-   **判断终止条件：** 函数首先评估当前区域是否需要进一步分割。递归将根据以下任一条件停止：
    *   当前区域的递归深度达到预设的最大值（`max_depth`，在实现中设为3）。
    *   区域内有效数据点的数量低于预设的最小网格数（`min_cells`，在实现中设为200）。
    *   当前区域内计算得到的坡向方差（使用 `np.var` 函数计算）低于预设的阈值（`aspect_variance_threshold`，在实现中设为0.7）。
    一旦满足上述任一条件，当前区域即被视为一个均匀的子区域，并作为最终的无重叠矩形区域返回。

-   **执行区域分割：** 如果不满足任何终止条件，函数将对当前区域进行二分。为了保证分割后子区域的几何合理性，函数会计算当前区域的宽度和高度，并选择其中较长的一边进行精确的二等分（即沿着该维度取中点进行分割）。例如，若当前区域在X轴方向上更长，则会沿X轴中点将其一分为二。随后，函数将对这两个新生成的子区域递归调用自身，重复上述判断和分割过程，直至所有子区域都满足终止条件。
...
局部坐标系的建立基于平面拟合所得的地形参数。根据平面方程 $z = \beta_0 + \beta_1 x + \beta_2 y$，区域的坡向角通过梯度向量的反向角确定：

$$\phi_k = \arctan2(-\beta_2, -\beta_1)$$

该角度指向海底坡面的最陡下降方向。由问题三得到的测线设计原则要求测线方向与等深线平行，即与坡向垂直，因此主测线方向角为：

$$\theta_k = \phi_k + \frac{\pi}{2}$$

基于此几何关系建立局部坐标系，其中u轴沿等深线方向，v轴沿坡向。全局坐标$(x,y)$到局部坐标$(u,v)$的变换通过二维旋转矩阵实现：
...
该变换的物理意义在于将原本弯曲的等深线在局部坐标系中近似为平行于u轴的直线，使得测线布设可以沿v轴方向进行规则化迭代。

### 3.2 变换参数计算与有效域确定

对于每个子区域，通过变换其矩形边界的四个角点获得局部坐标系下的有效测量域。设区域边界为$[x_{min}, x_{max}] \times [y_{min}, y_{max}]$，四个角点$(x_{min}, y_{min})$、$(x_{min}, y_{max})$、$(x_{max}, y_{min})$、$(x_{max}, y_{max})$经变换后得到对应的局部坐标。

通过统计变换后坐标的极值确定有效布线范围：
$$u_{min} = \min\{u_i\}, \quad u_{max} = \max\{u_i\}$$
$$v_{min} = \min\{v_i\}, \quad v_{max} = \max\{v_i\}$$

其中$v$轴范围$[v_{min}, v_{max}]$构成了测线迭代布设的边界约束。实际计算结果显示，7个子区域的主测线方向角度范围从-86.1°到258.8°，充分体现了各区域地形走向的多样性。局部坐标系的v轴范围从1.2海里到2.4海里不等，为后续的自适应测线间距计算提供了精确的几何边界。

这种基于地形特征的坐标变换方法不仅保持了数学上的严谨性，更重要的是通过几何直觉与物理意义的结合，为复杂地形下的测线优化设计提供了可行的数学框架。变换后的局部坐标系中，测线布设问题简化为沿单一方向的一维优化问题，显著降低了算法复杂度。
...
此阶段的核心目标是根据局部地形精确计算测线的**最优间距**与**平行走向**，其算法逻辑主要由 `generate_survey_lines.py` 脚本实现。

**1. 局部坐标系与动态水深模型**

我们将复杂海域划分为7个地形趋势相对单一的子区域，并为每个区域建立独立的局部坐标系 `(u, v)`，将三维空间中的规划问题降维至二维平面。为获取任意测线位置的精确水深，我们利用附件数据构建了一个 `LinearNDInterpolator` 线性插值器。在迭代布线时，可通过 `get_avg_depth_on_line` 函数对任意测线进行采样，计算其平均水深 `D_k`，从而实现对地形的动态感知。

**2. 基于和问题三一致的迭代间距模型**

和问题三一致：当海底存在坡度 `α` 时，多波束声呐的覆盖在迎坡（upslope）和顺坡（downslope）方向上是不对称的。考虑从换能器到海底的垂直距离 `D`，以及换能器开角 `θ=120°`，上坡和下坡方向的**斜距** `W_slant` 分别为：

$$ W_{\text{上坡}} = \frac{D \sin(\theta/2)}{\cos(\theta/2 + \alpha)}, \quad W_{\text{下坡}} = \frac{D \sin(\theta/2)}{\cos(\theta/2 - \alpha)} $$

该模型正确地反映了上坡侧覆盖短、下坡侧覆盖长的物理现实。为维持 `10%` 的目标重叠率 `η`，下一条测线 `k+1` 相对于当前测线 `k` 的布设距离 `d_k` 必须被精确计算。在 `calculate_line_distance` 函数中，我们通过综合几何推导，得到如下迭代公式：

$$ d_k = \frac{(W_{\text{上坡}} + W_{\text{下坡}}) (1 - \eta) \cos(\alpha)}{1 + (1 - \eta) \sin(\alpha) \frac{\sin(\theta/2)}{\cos(\theta/2 + \alpha)}} $$

此公式综合考虑了斜距、水平投影 `cos(α)` 以及重叠区域在坡度影响下的几何关系。算法从每个区域的坡向起始边界 `v_min` 开始，利用此公式循环计算 `d_k` 并生成新测线，直至完全覆盖 `v_max` 边界。

### 4.2 第二阶段：边界智能延伸优化

第一阶段生成的测线簇虽然间距最优，但在区域边界，特别是角落处，如果仅将测线延伸至和**矩形边界**交点处，由于覆盖范围的几何形状，和测线和边界之间存在角度，并不严格垂直，因此会产生漏测。第二阶段由 `generate_survey_lines_simple_optimized.py` 脚本执行，通过一个多层次的动态模型对每条测线的端点进行智能延伸，以确保覆盖的完整性。

**1. 延伸距离的动态几何模型**

该策略的核心在于 `find_line_region_intersections_extended` 函数，它为测线与区域边界的每个交点计算一个动态的延伸距离 `L_ext`，该距离由三个部分加和而成：

$$ L_{\text{ext}} = \text{基础延伸量} + \text{角度修正量} + \text{安全边距} $$

-   **基础延伸量**：设定为交点处的**完整覆盖宽度** $W_{full} = 2D_{boundary}\tan(\frac{\theta}{2})$，确保测线有足够的能力扫过边界。
-   **角度修正量**：当测线与边界夹角过小时，需要更大的延伸来覆盖角落。我们为此设计了一个角度修正系数 $\gamma$，其大小与测线方向向量和边界 法向量夹角的余弦绝对值成反比（$\gamma \propto 1/|\cos\phi|$），有效解决了边角漏测问题。
-   **安全边距**：在上述基础上，额外增加 `20%` 的覆盖宽度作为冗余，以对抗插值误差，确保覆盖的鲁棒性。

**2. 保守延伸策略的数学表达**

综合上述三个要素，最终的延伸距离计算公式为：

$$ L_{extension} = W_{full} + W_{full} \cdot \frac{(\gamma - 1)}{2} + 0.2 \cdot W_{full} $$
